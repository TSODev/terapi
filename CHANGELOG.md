...
### Fixed
- **Response viewer became almost unusable (~1 char/s typing) with large responses** — the JSON tree (`json_highlight::rows()`) was re-parsed and re-flattened from scratch on every render frame, and rendering runs on every keystroke regardless of which tab is focused (the Response panel is always visible alongside the Request sub-tabs). For a ~140k-row response (e.g. `geo.api.gouv.fr/communes`, all French municipalities), this alone cost ~420ms per keystroke in a debug build — reproduced with a synthetic same-size payload. Fixed in two parts: (1) `App::response_rows` now caches the flattened rows, rebuilt only at the small number of places `response_body`/`response_folds`/`response_headers` actually change (new response arrives, fold/unfold, new request, collection/history load…), not on every render; (2) even with a warm cache, `ui.rs` was still building a ratatui `Row`/`Cell` object for **every** row on every frame instead of just the visible ~30-40 — it now windows to the visible slice, replicating ratatui `Table`'s own cursor-follows-scroll logic (`get_row_bounds`) since a pre-sliced `Vec` can't rely on `TableState`'s built-in windowing. Combined: ~420ms → ~6.5ms per keystroke on the 140k-row benchmark (~50x).
